...
class libraryKakao extends socialxeLibrary
{
    ...
    function createAuthUrl($type)
    {
        ...
        $params = [
            ...
            'redirect_uri' => getNotEncodedFullUrl('', 'module', 'socialxe', 'act', 'procSocialxeCallback', 'service',
                'kakao'),
    ...
    function checkLinkage()
    {
        ...
        if ( ! $this->getAccessToken() || ! $user = $this->requestAPI('v1/api/story/isstoryuser', [],
                $this->getAccessToken())) {
            return new BaseObject(-1, 'msg_errer_api_connect');
        }
    ...
    function post($args)
    {
        ...
        $this->requestAPI('v1/api/story/post/note', ['content' => $args->title.' '.$args->url],
            $this->getAccessToken());
    }
